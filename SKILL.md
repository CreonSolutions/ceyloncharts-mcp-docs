...
9. **`get_quotes`' ambiguous handling is per-row, not whole-response**,
   ...
   returns `200` with CSV rows for the symbols that resolved — the bad one
   just comes back with empty `price`/`prevClose`/`change`/`changePct`/`asOf`
   fields in its row, rather than replacing the entire response with the
   candidates shape. A trailing note after the table (not the usual note
   prepended before it) says which symbols were not found or ambiguous, with
